...
prebuild_uboot()
{
	echo "##########################: $RK_TRUST_INI_CONFIG $RK_SPL_INI_CONFIG $RK_UBOOT_SIZE_CONFIG"
	...
	echo "##########################: $RK_TRUST_INI_CONFIG $RK_SPL_INI_CONFIG $RK_UBOOT_SIZE_CONFIG $UBOOT_COMPILE_COMMANDS"
}
...
usage()
{
	...
	echo "quectelcm          -build quectelcm"
	...
	echo "debian-rebuild     -re-build debian rootfs"
...
build_quectelcm()
{
	...
	# build binary
	make -C $QUECTELCM CC=$TARGET_CC
	chmod 755 $QUECTELCM/quectel-CM
	cp $QUECTELCM/quectel-CM $TARGET_ROOTFS_DIR/usr/bin/
	# copy linux-ppp-scripts
	cp $TOP_DIR/external/linux-ppp-scripts $TARGET_ROOTFS_DIR/ -rf
...
build_debian()
{
	...
	#LAN=CN VERSION=debug ARCH=$ARCH ./mk-rootfs-$RK_DEBIAN_VERSION.sh
	./mk-image.sh
